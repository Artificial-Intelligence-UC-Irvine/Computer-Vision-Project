...
def run_gesture_app():
    ...
    while True:
        ...
        # ✅ Fix 1: wrap in mp.Image — detector.detect() expects this, not raw numpy
        ...
        # Gesture detection
        # ✅ Fix 2: new API uses results.hand_landmarks, not results.multi_hand_landmarks
        if results.hand_landmarks:
            for hand_lms in results.hand_landmarks:

                # ✅ Fix 3: draw_landmarks_on_image takes the rgb numpy array from
                #           mp_image.numpy_view(), not frame.numpy_view()
                #           and returns RGB, so convert back to BGR for OpenCV
                annotated = draw_landmarks_on_image(mp_image.numpy_view(), results)
                frame = cv2.cvtColor(annotated, cv2.COLOR_RGB2BGR)

                # ✅ Fix 4: new API landmarks are plain objects, no .landmark sub-list
